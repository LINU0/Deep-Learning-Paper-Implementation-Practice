...
凍結原始預訓練權重 $W \in \mathbb{R}^{d \times d}$，注入兩個低秩矩陣：
- **矩陣 A**（降維）：$\mathbb{R}^{d \times r}$，以 $\mathcal{N}(0, \sigma^2)$ 初始化
- **矩陣 B**（升維）：$\mathbb{R}^{r \times d}$，以 **0** 初始化（確保訓練初期不破壞原始輸出）

最終輸出：$h = Wx + BAx$，其中只有 $A$、$B$ 被更新。
