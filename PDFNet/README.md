# HW2 — High-Precision Dichotomous Image Segmentation via Depth Integrity-Prior and Fine-Grained Patch Strategy
...
本作業以 PDFNet 論文（已錄取於 CVPR 2026）為基礎，目標為理解模型架構、考慮其景深特性，將模型遷移至醫學影像資料集 Kvasir-SEG。
...
*圖片來源：[arXiv:2503.06100](https://arxiv.org/abs/2503.06100)*
...
### `models/PDFNet.py`
- (舊版做法)
- Depth Decoder 的特徵融合方式為**逐元素相加（addition）**
- `shallow` 卷積的輸入從為 `raw_ch`（僅 RGB）
- 調整 LayerNorm 的位置 Pre-Norm
...
- 由於難以收斂，嘗試關閉部分資料增強（random perspective、affine、background 置換、Gaussian noise），減少訓練時的不穩定因素
