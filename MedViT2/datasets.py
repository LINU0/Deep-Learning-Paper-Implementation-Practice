...
def build_dataset(args):
    ...
    if args.dataset == 'Kvasir':
        ...
        return train_dataset, test_dataset, nb_classes
    # 新增自己的資料集
    elif args.dataset == 'Bone_Multiclass':
        data_dir = './Bone_Fracture_Multiclass'
        train_path = os.path.join(data_dir, 'train')
        test_path = os.path.join(data_dir, 'test')

        # 計算類別數量
        nb_classes = len([name for name in os.listdir(train_path) if os.path.isdir(os.path.join(train_path, name))])
        print(f"{nb_classes} 個類別。")
